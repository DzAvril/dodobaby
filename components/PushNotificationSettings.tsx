...
type DeviceStatus = "checking" | "unsupported" | "denied" | "unsubscribed" | "subscribed";
...
export function PushNotificationSettings({ initialSettings }: { initialSettings: Settings }) {
  ...
  async function subscribe() {
    ...
    try {
      ...
      const current = await registration.pushManager.getSubscription() ?? await registration.pushManager.subscribe({
        ...
        applicationServerKey: applicationServerKey(settings.vapidPublicKey),
      });
  ...
  async function sendTest() {
    ...
    setTestSent(false);
    try {
      await jsonRequest("/api/notifications/test", {
        method: "POST",
        headers: { "content-type": "application/json" },
      ...
      setError(caught instanceof Error ? caught.message : "测试通知发送失败");
    } finally {
      setPending(null);
    }
  }
  ...
  return (
    <form className="settings-section push-notification-settings" onSubmit={save}>
      ...
      <div className="push-notification-actions">
        ...
        {deviceReady ? <button type="button" className="secondary-button" disabled={pending !== null} onClick={sendTest}><Send />{pending === "test" ? "发送中…" : "发送测试通知"}</button> : <button type="button" className="primary-button" disabled={pending !== null || deviceStatus === "unsupported" || deviceStatus === "denied"} onClick={subscribe}><Bell />{pending === "subscribe" ? "开启中…" : "开启当前设备通知"}</button>}
