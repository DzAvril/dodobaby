...
一个适合家庭共同使用的宝宝照护记录网站。首页只提供跨模块概览，辅食、喂养、睡眠、尿布、用药、生长、疫苗与设置均使用独立页面；当前支持辅食月历、多餐与反应记录，亲喂与瓶喂时间线和间隔提醒，跨午夜睡眠，尿布观察，用药计划与实际服用记录，体重、身长/身高、头围趋势与 WHO 标准曲线对比，以及家庭接种计划与实际接种事实记录。手机底栏的三个高频模块可以在设置中调整。
...
登录后可在“设置 → 喂奶提醒”配置距上次喂奶多久后提醒，并分别为每台手机或电脑开启通知。VAPID 密钥由服务端首次使用时自动生成并保存在应用数据库中；VAPID 联系地址默认使用公开 HTTPS 的 `APP_URL`，也可通过 `DODOBABY_VAPID_SUBJECT` 单独配置为公开域名的 `mailto:` 或 HTTPS 地址。不要使用 localhost、`.local` 或内网 IP，Apple 会拒绝这类 VAPID 身份。容器内通知 worker 每分钟检查最新喂养记录，同一条记录对每台设备只发送一次。

iPhone 和 iPad 需使用 iOS/iPadOS 16.4 或更高版本，先把网站添加到主屏幕，再从主屏幕打开“小芽日记”并点击“开启当前设备通知”。Android 和支持标准 Web Push 的桌面浏览器可直接按浏览器提示授权。
...
CI 还会启动实际 Docker 容器，在隔离数据库中验证登录保护、同源拦截、PWA 通知设置、生长、睡眠、尿布、喂养与疫苗记录 CRUD、真实 PDF 导出，以及桌面和手机端的关键浏览器交互。
